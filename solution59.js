...
function funnyString (s) {
  ...
  for (let i = 0, itotal = values.length - 1; i < itotal; i++) {
    const a = Math.abs(values[i] - values[i + 1])
    console.log('a', values[i], values[i + 1])
    const b = Math.abs(values[count - i] - values[count - i - 1])
    console.log('b', values[count - i], values[count - i - 1])
    if (a !== b) {
      return 'Not Funny'
    }
  }
