...
function alternate (s) {
  ...
  const uniqueWord = [...new Set(s)]
  const validString = []
  let maxLength = 0

  for (let i = 0; i < uniqueWord.length; i++) {
    for (let j = i + 1; j < uniqueWord.length; j++) {
      const str = uniqueWord[i] + '|' + uniqueWord[j]
      const pattern = new RegExp(str, 'g')
      validString.push(s.match(pattern))
    }
  }

  for (let i = 0; i < validString.length; i++) {
    for (let j = 1; j < validString[i].length; j++) {
      if (validString[i][j - 1] === validString[i][j]) {
        validString.splice(i, 1)
        i--
        break
      }
      if (j + 1 === validString[i].length) {
